...
/** Right-hand panel listing everything pinned here (§5 Phase 3). Opened via ?panel=pins. */
export function PinsPanel({
  ...
  const { closePanel, openThread } = useThreadNav();
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape" && !(e.target instanceof HTMLElement && e.target.closest(".tiptap"))) closePanel();
    };
  ...
  return (
    <aside className="flex w-[400px] shrink-0 flex-col border-l border-border bg-background" aria-label="Pinned messages">
      <header className="flex h-12 shrink-0 items-center gap-2 border-b border-border px-4">
        <h2 className="text-[15px] font-semibold tracking-tight">Pinned</h2>
        <span className="min-w-0 truncate text-[13px] text-muted-foreground">
          {items.length > 0 ? `${items.length} in ${containerLabel}` : containerLabel}
        </span>
        <Tooltip>
