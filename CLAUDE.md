...
- Web push notifications for DMs, mentions, and every message in a channel whose `notification_level` is `all` — the level a channel is joined with, and what Settings promises ("Every message in every channel you have joined"); muted channels stay silent and Do Not Disturb hours silence everything. One rule in `lib/utils/notify-rules.ts` decides for the in-app notifier and the push fan-out alike; in-app toast when the window is open but a different channel is focused
...
- **Sign-in goes through the system browser, straight to Google.** Google refuses embedded browsers, so the app opens `/auth/google?next=/desktop/handoff`, which starts the OAuth flow and lands the browser on Google's account chooser; `/login` forwards there too, for apps installed before that route existed. Starting it must happen in a Route Handler or Server Action: PKCE writes a one-time cookie, and a page render cannot. `/desktop/handoff` mints a one-time sign-in token for the signed-in user (admin `generateLink`, no email sent), parks it under a one-shot id in `desktop_handoffs` (service-role only, two-minute expiry) and opens `commune://auth?handoff=<id>`; `/api/desktop/session` claims it and verifies it into a session of the app's own. Never put a token in the deep link.
...
- **Screen share uses Commune's picker, never the macOS system one.** The system picker only offers one window or the entire screen. Ours lists every screen and window and needs macOS Screen Recording permission, which it explains when missing. Refuse a request with `callback(null)`: an empty object makes Electron 44 throw, and the page reports either refusal as an AbortError, which the huddle treats as a change of mind.
- **A hidden window must survive and come back.** `backgroundThrottling` is off, because the page holds the realtime connection and rings for new messages while hidden. macOS can still reclaim a renderer left hidden for hours, and the window then comes back blank with its old title, so `render-process-gone` reloads and showing or focusing the window reloads a page with no text on it. Both are rate limited against reload loops.
- **Verify with `pnpm smoke` in `desktop/`** against the dev server before shipping the shell; it covers sign-in, badge, notifications, screen share, close-to-hide and external links. Packaged builds: `pnpm dist:mac`, `pnpm dist:win`, `pnpm release` (see `desktop/README.md`). The release publishes its own update feed through `pnpm manifests`, because electron-builder's publisher drops `latest*.yml` when the release already exists, and an update nobody is told about reaches nobody.
